...
### Auto-clicker:
```
javascript:(function(){if(document.getElementById('topbarOrteil')==null){alert('This only works on Cookie Clicker!');}else{if(typeof autoClick==='undefined'){window.autoClick=false;}if(!autoClick){PlaySound('snd/tick.mp3');autoClick=true;Game.Notify('Auto-Clicker Enabled','<b>MWA HA HA HA, it doesn\'t even know that I\'m cheating</b>',[1,7]);window.autoClickInterval=setInterval(()=>{Game.ClickCookie();},10);}else{PlaySound('snd/tickOff.mp3');autoClick=false;Game.Notify('Auto-Clicker Disabled','<b>NOOOOOOOOOOOOOOOOOOO</b>',[1,7]);clearInterval(window.autoClickInterval);}}})();
```
### Dev Tools:
```
